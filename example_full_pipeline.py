...
import time
import glob
from src.manuscript.detectors.east import EASTInfer
...
def main():
    # Пути к моделям
    recognizer_model_path = "C:\\Users\\USER\\Desktop\\OCR_MODELS\\exp1_model_64\\best_acc_weights.pth"
    ...
    for i, image_path in enumerate(image_files, 1):
        ...
        try:
            ...
            print(f"   📊 Найдено: {words_in_image} областей, распознано: {recognized_in_image} слов")
            
            # Промежуточная статистика каждые 10 изображений
            if i % 10 == 0:
                avg_time = total_processing_time / total_images if total_images > 0 else 0
            ...
            if recognized_in_image > 0:
                ...
                print(f"   🏃 Скорость: {words_per_sec:.1f} слов/сек, {time_per_word:.1f} мс/слово")
            ...
            if len(failed_images) >= 5 and len(failed_images) > total_images:
                print(f"⚠️ Слишком много ошибок ({len(failed_images)}). Возможна проблема с моделью!")
                break
    ...
    if total_images > 0:
        ...
        if total_words_recognized > 0:
            ...
            overall_time_per_word = total_processing_time / total_words_recognized * 1000
            recognition_rate = (total_words_recognized / total_words_found) * 100 if total_words_found > 0 else 0
            
            print(f"🏃 Общая скорость распознавания: {overall_words_per_sec:.1f} слов/сек")
